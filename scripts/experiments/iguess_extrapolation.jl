...
function instrumented_run(
        problem, method; solver, initialguess, bound = 1e3, max_iterations = 100)
...
function measure(make_problem, hamiltonian, method, ::Type{T},
        iguess; solver, max_iterations, cap) where {T}
    prob = make_problem(T)
    try
...
function ho_make(::Type{T}) where {T}
    ho_pode(T.(HO.q₀), T.(HO.p₀); timespan = (T(0.0), T(10_000.0)), timestep = T(1.0))
end  # nt = 10000
function pd_make(::Type{T}) where {T}
    pd_pode(T.(PD.q₀), T.(PD.p₀); timespan = (T(0.0), T(10_000.0)), timestep = T(1.0))
end  # nt = 10000
function dp_make(::Type{T}) where {T}
...
for (plabel, make, ham, solver) in PROBLEMS
    ...
    for (mname, method) in IMPLICIT_METHODS
        for T in PRECISIONS
            mh = measure(make, ham, method, T, fresh_iguess("Hermite");
                solver, max_iterations = CAP, cap = CAP)
            mm = measure(make, ham, method, T, fresh_iguess("Midpoint");
                solver, max_iterations = CAP, cap = CAP)
