...
# t = 10_000 (nt = 10_000 steps), at every precision — the local-frame stepping of
# `integrate_bounded` keeps the half precisions on a resolvable grid where a T-typed global clock
# would stall (t + 1 == t past t ≈ 2048 in Float16, t ≈ 256 in BFloat16). The solution error is
# measured against a Float64 Gauss(8) reference computed at the *fine* step (Δt_ref = 0.1, as in the
# short scenario) and subsampled onto each run's grid, so the reference is trustworthy independent
# of the coarse step; the reference integration is guarded so the energy-error plot is still
