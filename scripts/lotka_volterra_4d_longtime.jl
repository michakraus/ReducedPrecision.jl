# Reduced-precision study: Lotka–Volterra 4D (degenerate Lagrangian), coarse-step variant.
#
# Same pipeline as lotka_volterra_4d.jl but with a coarser timestep Δt = 0.1 and a longer horizon
...
plot_energy_error(runs, ham; groups = LV4D_GROUPS,
    path  = joinpath(plotdir, "lotka_volterra_4d_energy_error_dt=$(Δt).png"),
    title = "Lotka–Volterra 4D — Relative Energy Error (Δt = 0.1, t ≤ 100)")

if reference !== nothing
    plot_solution_error(runs, reference; groups = LV4D_GROUPS,
        path  = joinpath(plotdir, "lotka_volterra_4d_solution_error_dt=$(Δt).png"),
        title = "Lotka–Volterra 4D — Solution Error (Δt = 0.1, t ≤ 100, vs. Float64 Gauss(8))")

    plot_solution(runs; reference = reference, groups = LV4D_GROUPS,
        path   = joinpath(plotdir, "lotka_volterra_4d_solution_dt=$(Δt).png"),
        title  = "Lotka–Volterra 4D — Configuration-Space Trajectory (Δt = 0.1, t ≤ 100)",
        xlabel = "q₁", ylabel = "q₂")
end
